...
当前这个页面每个表格都有一个单独的保存按钮，改为使用一个统一的保存按钮，每次点击保存时，保存全部数据

|      |        |         |      |         |       |
| ---- | ------ | ------- | ---- | ------- | ----- |
| 美元 | 人民币 | 存款    | 基金 | 养老金  | 股票  |
| usd  | rmb    | deposit | fund | pension | stock |

#### 0518

在修改数据后保存时，除了在*_Data.ts内保存外，还应该生成app_totals_XXX.json的文件

假如我有本金1000元，基金投资了1000元，当前的收益是10元，也就是总本金是2000元，收益10元，本月基金分红了30元，从资产转移的角度，如何定义分红

答：分红是从基金变为了现金，份额不变，本金变少

假如我基金投资了1000元，当前的收益是10元，此时卖出了一半的仓位，我的基金总金额是会变成505还是需要补充基金净值才能计算

答：基金会变为505，



新增交易记录按钮，具体的功能包括，申购（本金增加），分红（本金减少），卖出（实际金额变化）

#### 存在的问题

1、本金增加，资金赎回时，如何准确计算收益的变化

2、新增数据时，存入到文件的数据没有收益率和持有天数，需要手动调整
...
本金1000，收益10元，此时分红了30元

那么会首先从收益扣除10元，然后从本金扣除20元

月份	支付宝	网商银行	工行银行	腾讯自选股	招商银行	总计

2025-05-20	51951.2  80102.68  101425.03  31253.28  54580.74  319312.93

新增表格在src\components\Dashboard.tsx，按照上面的格式，每一个名称都是列名，数据来源参考src\components\charts\TrendChart.tsx获取数据的方式

月份就是json文件上面的日期，

支付宝/网商银行/工行银行/腾讯自选股/招商银行 显示的是各自的total

最后的总计显示的是grand_total

数据格式放入到src\data\dataTypes.ts，

表格样式可以参考src\components\charts\TrendChart.tsx

新增表格，按照上面的格式，每一个名称都是列名